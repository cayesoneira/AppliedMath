 function [x,uh]=elfin1d(fp,fq,ff,a,b,datos_a,datos_b,nel,l,iopcoef,iopblo)
...
% l: orden de los polinomios de interpolación;
%   l grado de los polinomios de Lagrange, puede ser 1 ó 2
...
for k=1:nel
    ...
    if l==1
        if iopcoef==1
    Kk=fp/h*[1, -1;-1, 1];Mk=(fq*h)/6*[2 1;1 2];
    ck=ff*h/2*[1;1];
    elseif iopcoef==2
    punto_medio=(x(k)+x(k+1))/2;
    p=feval(fp,punto_medio);q=feval(fq,punto_medio);
    Kk=p/h*[1, -1;-1, 1];Mk=(q*h)/4*[1 1;1 1];
    f=feval(ff,punto_medio);
    ck=f*h/2*[1;1];
    elseif iopcoef==3
    p1=feval(fp,x(k));p2=feval(fp,x(k+1));
    q1=feval(fq,x(k));q2=feval(fq,x(k+1));
    Kk=(p1+p2)/2/h*[1, -1;-1, 1];Mk=h/2*[q1 0;0 q2]; 
